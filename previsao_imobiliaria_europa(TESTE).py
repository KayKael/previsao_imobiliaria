...
# Dicionário de países e suas siglas
country_names = {
...
def train_and_evaluate(data, country, freq):
    ...
    euro_to_currency_conversion_rate = 1.0
    
    # Prever os preços futuros
    future_prices = forecast_growth(country_data[column_name].values, f'house_price_model_{freq}_{country}.keras', 5, n_lags, scaler, freq)
    
    # Converter os preços futuros para euros
    future_prices_euros = [price * euro_to_currency_conversion_rate for price in future_prices]
    
    # Exibir as previsões de preços futuros em euros
    print('\nPrevisões de Preços Futuros (em euros):')
    if freq == 'quarterly':
...
def user_selection(countries_of_interest, data_quarterly, data_annual, country_names):
    # Exibir a lista de países disponíveis
    print("Selecione um país:")
    for idx, country in enumerate(countries_of_interest, 1):
        print(f"{idx}. {country} ({country_names[country]})")
    ...
    print(f'\n\nProcessando país: {selected_country} ({country_names[selected_country]})')
    print('\nDados Trimestrais:')
...
user_selection(countries_of_interest, data_quarterly, data_annual, country_names)
